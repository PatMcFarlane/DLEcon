...
include(srcdir("p03_Functions_Bellman.jl"))      # Project-specific functions 
...
#@time vals1, vals2, vals3 = nn_train!(opt,m,epochs)

# Plot solution
wvec = collect(LinRange(wmin, wmax, 100)')
ζvec , hvec, vvec, dvvec = hh_policy_fn(m,zeros(typeof(prec),1,100),zeros(typeof(prec),1,100),zeros(typeof(prec),1,100),zeros(typeof(prec),1,100),wvec,dtype)
